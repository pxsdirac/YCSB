...
public class ScyllaCQLBatchClient extends DB {
  ...
  public static final String SUB_KEY = "sub_id";
  ...
  private static boolean isMultiPartitions = false;

  private static int batchWriteSize = 0;
  ...
  public void init() throws DBException {
    ...
    synchronized (INIT_COUNT) {
      ...
      try {
        ...
        isMultiPartitions = Boolean.parseBoolean(getProperties().getProperty("multiple-partition","false"));
        batchWriteSize = Integer.parseInt(getProperties().getProperty("batch-write-size","0"));
  ...
  public Status read(String table, String key, Set<String> fields,
    ...
    String realTable = isMultiPartitions ? table : table + "1";
    try {
      ...
      if (stmt == null) {
        ...
        if (isMultiPartitions){
          stmt = session.prepare(selectBuilder.from(realTable)
              .where(QueryBuilder.in(YCSB_KEY, QueryBuilder.bindMarker())));
        } else {
          stmt = session.prepare(selectBuilder.from(realTable)
              .where(QueryBuilder.eq(YCSB_KEY, QueryBuilder.bindMarker()))
              .and(QueryBuilder.in(SUB_KEY,QueryBuilder.bindMarker())));
        }
      ...
      if (isMultiPartitions){
        List<String> keys = readRecordsIndexes.stream().map(idx ->{
          String realKey = key + "_" + idx;
          return realKey;
        }).collect(Collectors.toList());
        boundStatement.setList(0,keys);

      } else {
        boundStatement.setString(0,key);
        List<String> subIds = readRecordsIndexes.stream().map(idx -> idx.toString()).collect(Collectors.toList());
        boundStatement.setList(1,subIds);
      }

      ResultSet rs = session.execute(boundStatement);

      List<Row> rows = rs.all();
      if (rows.size() != readRecordsSize){
        throw new IllegalStateException("records size " + rows.size() + " is not 200");
      }
...
//      LOGGER.error(MessageFormatter.format("Error reading key: {}", key).getMessage(), e);
  ...
  private <T> List<List<T>> group(List<T> list, int size){
    int groupNo = 0;
    List<List<T>> groups = new ArrayList<>();
    List<T> aGroup = new ArrayList<>(size);
    for (int i = 0; i < list.size(); i++) {
      ...
      aGroup.add(list.get(i));
    }
    if (!aGroup.isEmpty()) groups.add(aGroup);
    return groups;
  }
  /**
  ...
  public Status insert(String table, String key, Map<String, ByteIterator> values) {
    ...
    values.entrySet().forEach(entry -> evaluatedValues.put(entry.getKey(),entry.getValue().toString()));
    ...
    try {
      ...
      // Prepare statement on demand
      if (stmt == null) {
        Insert insertStmt = QueryBuilder.insertInto(realTable);

        // Add key
        insertStmt.value(YCSB_KEY, QueryBuilder.bindMarker());
        if (!isMultiPartitions){
          insertStmt.value(SUB_KEY,QueryBuilder.bindMarker());
        }
      ...
      final PreparedStatement immutableStmt = stmt;
      
      int batchSize = batchWriteSize == 0 ? allRecordsSize : batchWriteSize;

      group(allRecordsIndexes,batchSize).forEach(group ->{
        BatchStatement batchStatement = new BatchStatement(BatchStatement.Type.UNLOGGED);
        group.forEach(idx -> {
          // Add key
          BoundStatement boundStmt = immutableStmt.bind();

          if (isMultiPartitions){
            String realKey = key + "_" + idx;
            boundStmt.setString(YCSB_KEY, realKey);
          } else {
            boundStmt.setString(YCSB_KEY, key);
            boundStmt.setString(SUB_KEY,idx.toString());
          }

          evaluatedValues.forEach((featureName,value) -> {
            boundStmt.setString(featureName,value);
          });
